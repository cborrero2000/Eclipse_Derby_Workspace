...
/**
 * This class is utilities specific to the two ij Main's. This factoring enables
 * sharing the functionality for single and dual connection ij runs.
 * 
 */
public class utilMain {
	...
	/*
	 * command can be redirected, so we stack up command grabbers as needed.
	 */
	...
	/**
	 * Set up the test to run with 'numConnections' connections/users. This
	 * overload allows the choice of whether the system properties will be used
	 * or not.
	 *
	 * @param numConnections
	 *            The number of connections/users to test.
	 ...
	 *            The number of connections/users to test.
	 * @param ignoreErrors
	 *            A list of errors to ignore. If null, all errors are printed
	 *            out and nothing is fatal. If non-null, if an error is hit and
	 *            it is in this list, it is silently ignore. Otherwise, an
	 *            ijFatalException is thrown. ignoreErrors is used for stress
	 *            tests.
	 */
	public utilMain(int numConnections, LocalizedOutput out, Hashtable ignoreErrors) throws ijFatalException {
		/*
		 * init the parser; give it no input to start with. (1 parser for entire
		 * test.)
		 */
		charStream = new UCode_CharStream(new StringReader(" "), 1, 1);
		...
		showErrorCode = Boolean.valueOf(util.getSystemProperty("ij.showErrorCode")).booleanValue();
	...
	public void initFromEnvironment() {
		...
		for (int ictr = 0; ictr < numConnections; ictr++) {
			try {
				...
				JDBCDisplayUtil.ShowException(out, s); // will continue past
														// connect failure
			} catch (ClassNotFoundException c) {
				JDBCDisplayUtil.ShowException(out, c); // will continue past
														// driver failure
			} catch (InstantiationException i) {
				JDBCDisplayUtil.ShowException(out, i); // will continue past
														// driver failure
			} catch (IllegalAccessException ia) {
				JDBCDisplayUtil.ShowException(out, ia); // will continue past
														// driver failure
				...
				JDBCDisplayUtil.ShowException(out, ie); // will continue past
														// driver failure
			}
		}
	}
	 ...
	 * Any prior input and output will be lost.
	 *
	 * @param in
	 *            source for input to ij
	 * @param out
	...
	public void go(LocalizedInput[] in, LocalizedOutput out) throws ijFatalException {
		...
		if (firstRun) {
			...
			InputStream versionStream = util.getResourceAsStream(ProductGenusNames.TOOLS_INFO);
			...
			String version;
			if (ijVersion != null) {
				version = "" + ijVersion.getMajorVersion() + "." + ijVersion.getMinorVersion();
			} else {
				version = "?";
			}

			out.println(langUtil.getTextMessage("IJ_IjVers30C199", version));
			for (int i = connEnv.length - 1; i >= 0; i--) { // print out any
															// initial
															// warnings...
	...
	/**
	 * Support to run a script. Performs minimal setup to set the passed in
	 * connection into the existing ij setup, ConnectionEnv.
	 * 
	 * @param conn
	 * @param in
	 */
	public int goScript(Connection conn, LocalizedInput in) {
		...
		return runScriptGuts(null);
	}

	private void supportIJProperties(ConnectionEnv env) {
		// check if the property is set to not show select count and set the
		// static variable
		// accordingly.
		...
		// check if the property is set to not show initial connections and
		// accordingly set the
		// static variable.
	...
	/**
	 * Run the guts of the script. Split out to allow calling from the full ij
	 * and the minimal goScript.
	 * 
	 * @param in
	 * @return The number of errors seen in the script.
	 *
	 */
	private int runScriptGuts(LocalizedInput[] in) {
		...
		while (!ijParser.exit && !done) {
			...
			try {
				command = null;
				out.flush();
				command = commandGrabber[currCE].nextStatement(out, fileInput);
				...
				if (command == null && oldGrabbers.empty()) {
					...
					if (fileInput) {
						...
						// SWITCHING THE INPUT FROM FILE TO KEYBOARD and testing it
						 //System.out.println("SWITCHING THE INPUT FROM FILE TO KEYBOARDSWITCHING THE INPUT FROM FILE TO KEYBOARD");
						 fileInput = false;
						 in = new LocalizedInput[] {langUtil.getNewInput(System.in) } ;
						 commandGrabber[0].ReInit(in[0]);
						 if(in[currCE].isStandardInput() == true)
						 {
							 //System.out.println(":) :) :) YES IT'S SYSTEM.IN :) :) :)");
						 }
						 else
						 {
							 //System.out.println(":( :( :( IT'S NOT SYSTEM.IN :( :( :( ");
						 }
					}
					...
					// if something went wrong, an SQLException or ijException
					// was thrown.
					// we can keep going to the next statement on those (see
					// catches below).
	...
	/**
	 * Perform cleanup after a script has been run. Close the input streams if
	 * required and shutdown derby on an exit.
	 * 
	 * @param in
	 */
	private void cleanupGo(LocalizedInput[] in) {
		...
		for (int i = 0; i < numConnections; i++) {
			try {
				...
				out.println(langUtil.getTextMessage("IJ_CannotCloseInFile", e.toString()));
			}
		}
	...
	private void displayResult(LocalizedOutput out, ijResult result, Connection conn) throws SQLException {
		...
		if (result != null) {
			if (result.isConnection()) {
				...
				try {
					util.DisplayMulti(out, (PreparedStatement) result.getStatement(), result.getResultSet(),
				...
				try {
					JDBCDisplayUtil.DisplayResults(out, rs, connEnv[currCE].getConnection(),
							result.getColumnDisplayList(), result.getColumnWidthList());
				} catch (SQLException se) {
					result.closeStatement();
					throw se;
				}
				...
				try {
					JDBCDisplayUtil.DisplayMultipleResults(out, resultSets, connEnv[currCE].getConnection(),
							result.getColumnDisplayList(), result.getColumnWidthList());
				} catch (SQLException se) {
					result.closeStatement();
					throw se;
	...
	/**
	 * catch processing on failed commands. This really ought to be in ij
	 * somehow, but it was easier to catch in Main.
	 */
	private boolean doCatch(String command) {
		...
		try {
			...
			if (elapsedTimeOn) {
				endTime = System.currentTimeMillis();
				out.println(
						langUtil.getTextMessage("IJ_ElapTime0Mil_4", langUtil.getNumberAsString(endTime - beginTime)));
			}
			return true;
	...
	/**
	 * This routine displays SQL exceptions and decides whether they are fatal
	 * or not, based on the ignoreErrors field. If they are fatal, an
	 * ijFatalException is thrown. Lifted from ij/util.java:ShowSQLException
	 */
	private void handleSQLException(LocalizedOutput out, SQLException e) throws ijFatalException {
		...
		if (showErrorCode) {
			errorCode = langUtil.getTextMessage("IJ_Erro0", langUtil.getNumberAsString(e.getErrorCode()));
		} else {
			errorCode = "";
		}
		...
		for (; e != null; e = e.getNextException()) {
			...
			/*
			 ** If we are to throw errors, then throw the exceptions that aren't
			 * in the ignoreErrors list. If the ignoreErrors list is null we
			 * don't throw any errors.
			 */
			if (ignoreErrors != null) {
				if ((sqlState != null) && (ignoreErrors.get(sqlState) != null)) {
					continue;
				} else {
					fatalException = e;
				}
	...
	void newInput(String fileName) {
		...
		commandGrabber[currCE] = new StatementFinder(
				langUtil.getNewInput(new BufferedInputStream(newFile, BUFFEREDFILESIZE)), null);
		fileInput = true;
	}
	...
	void newResourceInput(String resourceName) {
		...
		oldGrabbers.push(commandGrabber[currCE]);
		commandGrabber[currCE] = new StatementFinder(
				langUtil.getNewEncodedInput(new BufferedInputStream(is, BUFFEREDFILESIZE), "UTF8"), null);
		fileInput = true;
	}

	/**
	 * REMIND: eventually this might be part of StatementFinder, used at each
	 * carriage return to show that it is still "live" when it is reading
	 * multi-line input.
	 */
	 ...
	 * @param row
	 *            The row # to move to. (Negative means from the end of the
	 *            result set.)
	 *
	 * @return NULL.
	 *
	 ...
	 *            The specified ResultSet.
	 * @param row
	 *            The # of rows to move. (Negative means toward the beginning of
	 *            the result set.)
	 *
	 * @return NULL.
	 *
	...
	/**
	 * Position before the first row of the specified ResultSet and return NULL
	 * to the user.
	 *
	 * @param rs
	 *            The specified ResultSet.
	...
	/**
	 * Position on the first row of the specified ResultSet and return that row
	 * to the user.
	 *
	 * @param rs
	 *            The specified ResultSet.
	...
	/**
	 * Position after the last row of the specified ResultSet and return NULL to
	 * the user.
	 *
	 * @param rs
	 *            The specified ResultSet.
	...
	/**
	 * Position on the last row of the specified ResultSet and return that row
	 * to the user.
	 *
	 * @param rs
	 *            The specified ResultSet.
	...
	/**
	 * Position on the previous row of the specified ResultSet and return that
	 * row to the user.
	 *
	 * @param rs
	 *            The specified ResultSet.
